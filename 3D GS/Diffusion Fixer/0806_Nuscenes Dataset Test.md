...
由于Nuscenes数据传感器的频率无法做到帧精确匹配+点云稀疏+标注质量不高，其效果不如Waymo数据集的重建效果。
...
**阶段性结论**
1. 训练集和测试集指标差距过大，原因主要是数据集频率较低(2HZ)，即用第0秒和第1秒中间插值推测第0.5秒的渲染图像信息不充分，并且主车移速较高；
...
> b. 由于点云的稀疏性，导致***点云图较多缺失*** (具体见Fig.5)，对于Waymo数据集即使点云图中的点云尺寸设置较小依然能够得到较稠密的点云图；
...
### 2.3 第三阶段
在第三阶段，将原始数据和修复数据同时训练，在原始数据上相关指标如下：
...
https://github.com/user-attachments/assets/d397f7d9-165e-4111-8a99-b1e524ed5fb0

## 总结
通过本次试验，相关总结如下：
> a. 传感器数据/标注信息的准确性，点云数据的稠密性(每帧数量)对三维重建的结果有极大影响，需要尽量确保数据的质量；   
> b. 目前的修复器强依赖于点云的稠密性，后续有更多数据时可以重新训练；
