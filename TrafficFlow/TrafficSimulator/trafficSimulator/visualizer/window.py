...
class Window:
    def __init__(self, simulation):
        ...
        self.zoom_speed = 1
        self.Ncar = 0
        self.model_time_gap1 = 0
        self.model_time_gap2 = 0
        ...
        with dpg.window(
            ...
            with dpg.collapsing_header(label="Simulation Status", default_open=True):
                ...
                with dpg.table(header_row=False):
                    ...
                    with dpg.table_row():
                        dpg.add_text("CalTime1:")
                        dpg.add_text("_ms", tag="CTimeStatus1")
                    
                    with dpg.table_row():
                        dpg.add_text("CalTime2:")
                        dpg.add_text("_ms", tag="CTimeStatus2")
    ...
    def update_panels(self):
        ...
        if self.is_running:
            ...
            dpg.set_value("CarNumber", self.Ncar)
            dpg.configure_item("StatusText", color=(0, 255, 0))
        else:
        ...
        dpg.set_value("CTimeStatus1", f"{self.model_time_gap1:.2f}ms")
        dpg.set_value("CTimeStatus2", f"{self.model_time_gap2:.2f}ms")
    ...
    def render_loop(self):
        ...
        # Update simulation
        if self.is_running:
            time1,time2 = self.simulation.run(self.speed)
            self.model_time_gap1 = 10**3*(time1)
            self.model_time_gap2 = 10**3*(time2)
            
        temp = 0
        for segment in self.simulation.segments:
            temp += len(segment.vehicles)
        self.Ncar = temp
        #print(len(self.simulation.vehicles))
